...
class MessageHandler:
    ...
    def share(self, viber_request):
        viber.send_messages(viber_request.sender.id, [TextMessage(text='Кнопка Рассказать про меня')])
        botton_back(viber_request)
    ...
    def review(self, viber_request):
        photo = f'{self.request.scheme}://{self.request.get_host()}/media/qr-code.jpg'
        viber.send_messages(viber_request.sender.id, [PictureMessage(
            media=photo, text="Viber logo", thumbnail=photo,
        )])
    ...
    def command_not_found(self, viber_request):
        viber.send_messages(viber_request.sender.id, [TextMessage(text='Команда не найдена, выберите пункт меню')])
        botton_main_disp(viber_request)
